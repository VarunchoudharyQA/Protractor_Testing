...
var SetPasswordPage = require('setPassword.po.js');
var fs = require('fs');
...
testsUtils = {
    ...
    createSheduleUnit: function(name) {
    ...
    login: function (email, password) {
        var deferred = protractor.promise.defer();
        browser.driver.get(protractor.AUTH_APP_URL + '/login/#/?multiStage=true').then(function () {
            protractor.testUtils.waitForPage(protractor.AUTH_APP_URL + '/login/#/?multiStage=true').then(function () {
    ...
    loginOld: function (email, password) {
        var deferred = protractor.promise.defer();
        browser.driver.get(protractor.AUTH_APP_URL + '/login/#/?multiStage=false').then(function () {
            protractor.testUtils.waitForPage(protractor.AUTH_APP_URL + '/login/#/?multiStage=false').then(function () {
                element(by.model('user.email')).sendKeys(email).then(function () {
                    element(by.model('user.password')).sendKeys(password).then(function () {
    ...
    logout: function() {
        ...
        browser.refresh().then(function () {
            protractor.testUtils.waitUntilVisible(element(by.css('.user-drop-down .dropdown-toggle .arrow-img'))).then(function () {
                var logoutLink = element(by.id('Logout'));
                var loginPage = element(by.css('.login-page'));
                element(by.css('.user-drop-down .dropdown-toggle .arrow-img')).click().then(function () {
    ...
    fillUserFields: function (myemail, fieldsToFill, values) {
        ...
        if (fieldsToFill === undefined || this.arrayIncludes(fieldsToFill, 'emailAddress')) {
            promiseArray.push(element(by.model('user.emailAddress')).clear().sendKeys(myemail));
        }
        ...
        if (fieldsToFill === undefined || this.arrayIncludes(fieldsToFill, 'assignedGroup')) {
            var timeZones = new WfmDropdown(element(by.model('user.assignedGroup')));
            promiseArray.push(timeZones.selectItemByIndex(0));
        }
...
//assuming account was already created and state is userManagement
    createNewUser: function (myemail, values) {
        ...
        return browser.wait(function () {
            ...
            element(by.css('button#saveWithPopoverBtn')).click();
            return myemail;
        });
    },

    addUserWithRoleAndActivate: function(shouldLogout, orgName, adminCred, userCred, userRole, token) {
        // add new user with role to existing tenant and activate this user and perform login

        var deferred = protractor.promise.defer(),
            landingPage = userRole === 'Employee' ? '/#/mySchedule' : '/#/scheduler';

        protractor.testUtils.maximizeBrowserWindow();

        if (shouldLogout) {
            protractor.testUtils.logout();
        }

        protractor.testUtils.login(adminCred.email, adminCred.password, '/#/').then(function () {
            protractor.testUtilsNoUI.createNewUser(userCred.email, { role: userRole }).then(function () {
                protractor.testUtilsNoUI.inviteUserByEmail(userCred.email , adminCred.email, token).then(function() {
                    protractor.testUtils.logout();
                    protractor.testUtils.waitForPage('/login/#/').then(function() {
                        testEmailUtils.goToAccountActivationUrl(userCred.email, orgName).then(function() {
                            protractor.testUtils.waitForPage('/login/#/').then(function() {
                                new SetPasswordPage().resetPasswordAndSubmit(userCred.password);
                                protractor.testUtils.waitForPage(landingPage).then(function() {
                                    protractor.testUtils.maximizeBrowserWindow();
                                    deferred.fulfill();
                                });
                            });
    ...
    getRandomEmail: function (length) {
        ...
        return this.getFullRandomString(length || defaultLength).toLowerCase() + '.' + timestamp + '@mailinator.com';
    },
    ...
    gridUtils: {
        ...
        getRowNum: function(row){
            return element.all(by.css('.ag-row')).get(row);
        },
        ...
        getCellAt: function(row, cell){
            return element.all(by.css('.ag-row')).get(row).all(by.css('.ag-cell')).get(cell);
        },
        getRowContainTextInCol: function(text, colNumber) {
            return element.all(by.xpath('//div[contains(@class, "ag-row")]/div[contains(., "' + text + '") and contains(@col,"' + colNumber + '")]/..')).first();
        },
        getCellAtForRowElement:  function(ele, cellIndex) {
            return ele.all(by.css('.ag-cell')).get(cellIndex);
        },
        rowCount: function(){
            return element.all(by.css('.ag-row')).count();
        },
    ...
    isTestTheCurrentEnvironment: function() {
        return this.getCurrentEnvironment() === this.ENVIRONMENTS.test;
    },
